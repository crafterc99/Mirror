// MIRROROS — 10 core flow screens.
// Each screen is a function returning JSX that fills a parent rect (absolute inset:0).
// All assume mounted inside <IOSDevice dark>, so status bar / home indicator are handled.

// ─────────────────────────────────────────────────────────────
// Shared atoms
// ─────────────────────────────────────────────────────────────

function MOLogo({ size = 14, color = 'rgba(245,247,255,0.95)' }) {
  // Two interlocking discs — mirror facing self.
  const s = size;
  return (
    <svg width={s * 2.2} height={s} viewBox="0 0 22 10" fill="none" style={{ display: 'block' }}>
...
function MOWordmark({ size = 12, tracking = 0.32, color = 'rgba(245,247,255,0.95)' }) {
  return (
    <span style={{
      fontFamily: 'Geist, system-ui', fontWeight: 500, fontSize: size,
      letterSpacing: `${tracking}em`, color, textTransform: 'uppercase',
    }}>MIRROROS</span>
  );
}

function MOStatusGap() {
  // reserves vertical space below the iOS status bar / dynamic island
...
function MOPageDots({ total, active }) {
  return (
    ...
    <div className="mo-screen">
      ...
      <div style={{
        ...
        <div className="mo-serif" style={{
          fontSize: 56, lineHeight: 1, color: 'rgba(245,247,255,0.98)',
          letterSpacing: '-0.03em',
        }}>
          mirror<span style={{ fontStyle: 'italic', opacity: 0.85 }}>os</span>
        </div>
        <div style={{
          marginTop: 18, fontFamily: 'Geist', fontSize: 12, letterSpacing: '0.4em',
      ...
      {/* Subtle build version */}
      <div style={{
        position: 'absolute', bottom: 56, left: 0, right: 0, textAlign: 'center',
      }}>
        <span className="mo-mono" style={{
          fontSize: 10, color: 'rgba(200,210,230,0.32)', letterSpacing: '0.15em',
        }}>v 0.1 · ATELIER</span>
      </div>
    </div>
  );
...
// 02 · Intro / Philosophy
// ═════════════════════════════════════════════════════════════
function Screen02_Intro({ onNext }) {
  const [page, setPage] = React.useState(0);
  const pages = [
    {
      eyebrow: 'PRINCIPLE I',
      title: ['Become conscious of', 'who you are.'],
      body: 'MIRROROS notices the small currents of your inner life — what you feel, what you repeat, what you carry without naming.',
    },
    {
      eyebrow: 'PRINCIPLE II',
      title: ['Patterns reveal', 'themselves quietly.'],
      body: 'Across conversations and entries, a soft intelligence surfaces the shapes you live inside — gently, without verdict.',
    },
    {
      eyebrow: 'PRINCIPLE III',
      title: ['Grow on purpose.'],
      body: 'When you can see yourself clearly, change is no longer a struggle. It becomes the natural next breath.',
    },
  ];
  const p = pages[page];
  ...
  return (
    <div className="mo-screen">
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 24px' }}>
        <MOLogo size={11} color="rgba(245,247,255,0.7)" />
        <span style={{
      ...
      <div style={{ padding: '60px 30px 0', position: 'relative' }}>
        <span className="mo-mono" style={{
          fontSize: 10, letterSpacing: '0.3em',
          color: 'rgba(167,139,250,0.85)',
        }}>{p.eyebrow}</span>
        <h1 className="mo-serif" style={{
          margin: '20px 0 0', fontSize: 42, lineHeight: 1.05,
        ...
        <p style={{
          marginTop: 28, fontFamily: 'Geist', fontSize: 15, lineHeight: 1.55,
          color: 'rgba(220,225,240,0.65)', maxWidth: 280, fontWeight: 300,
        }}>{p.body}</p>
      </div>

      {/* Drifting ambient mark */}
      <div style={{
      ...
      <div style={{ position: 'absolute', bottom: 60, left: 0, right: 0, padding: '0 30px' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <MOPageDots total={pages.length} active={page} />
          <div
            className="mo-tap"
            onClick={() => page < pages.length - 1 ? setPage(page + 1) : onNext && onNext()}
            style={{
...
// 03 · Account Creation
// ═════════════════════════════════════════════════════════════
function Screen03_Account() {
  return (
    <div className="mo-screen">
      <div className="mo-ambient" style={{ opacity: 0.7 }} />
      <div className="mo-grain" />
      <div className="mo-vignette" />
      ...
      <div style={{ padding: '90px 30px 0' }}>
        ...
        <h1 className="mo-serif" style={{
          margin: '18px 0 0', fontSize: 42, lineHeight: 1.05,
          color: 'rgba(245,247,255,0.98)', letterSpacing: '-0.025em',
        }}>
          A space that's<br /><span style={{ fontStyle: 'italic' }}>only yours.</span>
        </h1>
        <p style={{
          marginTop: 20, fontFamily: 'Geist', fontSize: 14, lineHeight: 1.55,
          color: 'rgba(220,225,240,0.6)', maxWidth: 270, fontWeight: 300,
        }}>
      ...
      <div style={{
        ...
        <button className="mo-pill primary mo-tap" style={{ width: '100%' }}>
        ...
        <button className="mo-pill mo-tap" style={{ width: '100%' }}>
        ...
        <div style={{
          marginTop: 14, textAlign: 'center', fontFamily: 'Geist', fontSize: 11,
          color: 'rgba(200,210,230,0.4)', letterSpacing: '0.05em',
        }}>
...
// ═════════════════════════════════════════════════════════════
// 04 · Initial Emotional Intake
// ═════════════════════════════════════════════════════════════
function Screen04_Intake() {
  const [step, setStep] = React.useState(0);
  const [picks, setPicks] = React.useState([new Set(), new Set(), new Set()]);
  const questions = [
    {
      q: 'How do you feel most days?',
      eyebrow: '01 · Baseline',
  ...
  const toggle = (opt) => {
    ...
    if (next[step].has(opt)) next[step].delete(opt); else next[step].add(opt);
    setPicks(next);
  };
  ...
  return (
    <div className="mo-screen">
      ...
      <div style={{ padding: '32px 28px 0' }}>
        ...
        <h1 className="mo-serif" style={{
          margin: '14px 0 0', fontSize: 34, lineHeight: 1.1,
          color: 'rgba(245,247,255,0.98)', letterSpacing: '-0.02em',
        }}>{cur.q}</h1>
        <p style={{
          marginTop: 10, fontFamily: 'Geist', fontSize: 13, lineHeight: 1.5,
          color: 'rgba(220,225,240,0.5)', fontWeight: 300,
        }}>Tap as many as feel true. There is no wrong answer.</p>
      </div>

      <div style={{
        padding: '24px 22px 0', display: 'flex', flexWrap: 'wrap', gap: 10,
      }}>
        {cur.opts.map(opt => {
          ...
          return (
            <div
              key={opt}
              className={`mo-glass mo-tap ${on ? 'selected' : ''}`}
              onClick={() => toggle(opt)}
              style={{
      ...
      <div style={{
        ...
        <span style={{
          fontFamily: 'Geist', fontSize: 13, color: 'rgba(200,210,230,0.45)',
        }}>{sel.size} selected</span>
        <button
          className="mo-pill primary mo-tap"
...
function Screen05_Goals() {
  ...
  const [sel, setSel] = React.useState(new Set(['Find peace', 'Understand myself']));
  const toggle = (t) => {
    const n = new Set(sel); n.has(t) ? n.delete(t) : n.add(t); setSel(n);
  };
        ...
        {goals.map(g => {
          ...
          return (
            <div
              ...
              className={`mo-glass mo-tap ${on ? 'selected' : ''}`}
              onClick={() => toggle(g.t)}
              style={{ padding: 14, height: 110, position: 'relative', overflow: 'hidden' }}
            >
              {/* tiny constellation glyph */}
              <div style={{
              ...
              <div className="mo-serif" style={{
                marginTop: 28, fontSize: 18, lineHeight: 1.1,
                color: 'rgba(245,247,255,0.98)', letterSpacing: '-0.02em',
              }}>{g.t}</div>
              <div style={{
                marginTop: 6, fontFamily: 'Geist', fontSize: 11.5, lineHeight: 1.4,
                color: 'rgba(220,225,240,0.55)', fontWeight: 300,
              }}>{g.d}</div>
            </div>
          );
        })}
  ...
  return (
    <div className="mo-screen">
      ...
      <div style={{
        ...
        <div className="mo-orb" style={{ width: '100%', height: '100%' }} />
        {/* mic glyph */}
        <svg width="32" height="44" viewBox="0 0 32 44" style={{
      ...
      <div style={{
        position: 'absolute', bottom: 150, left: 32, right: 32, textAlign: 'center',
        fontFamily: 'Geist', fontSize: 13, lineHeight: 1.6,
        color: 'rgba(220,225,240,0.65)', fontWeight: 300,
      }}>
      ...
      <div style={{ position: 'absolute', bottom: 60, left: 28, right: 28, display: 'flex', flexDirection: 'column', gap: 10 }}>
        <button className="mo-pill primary mo-tap" style={{ width: '100%' }}>Enable microphone</button>
        <button className="mo-tap" style={{
          background: 'transparent', border: 0, color: 'rgba(200,210,230,0.55)',
          fontFamily: 'Geist', fontSize: 14, padding: 12, cursor: 'pointer',
  ...
  return (
    <div className="mo-screen">
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 22px 0' }}>
        <div>
          <div className="mo-mono" style={{ fontSize: 10, letterSpacing: '0.25em', color: 'rgba(200,210,230,0.5)' }}>
            TUESDAY · 6:24 PM
          </div>
          <div className="mo-serif" style={{
            marginTop: 6, fontSize: 26, lineHeight: 1.1,
            color: 'rgba(245,247,255,0.98)', letterSpacing: '-0.02em',
          }}>
      ...
      <div style={{ padding: '22px 18px 0' }}>
        <div className="mo-glass" style={{ padding: 18, position: 'relative', overflow: 'hidden' }}>
          <div style={{
            ...
            width: 140, height: 140, opacity: 0.7,
            background: 'radial-gradient(circle, rgba(167,139,250,0.5) 0%, transparent 60%)',
            filter: 'blur(8px)',
          }} />
          <div className="mo-mono" style={{ fontSize: 9.5, letterSpacing: '0.3em', color: 'rgba(167,139,250,0.85)' }}>
            CHECK · IN
          </div>
          <div className="mo-serif" style={{
            marginTop: 8, fontSize: 24, lineHeight: 1.15,
            color: 'rgba(245,247,255,0.98)', letterSpacing: '-0.02em',
          }}>
          ...
          <div style={{ marginTop: 18, display: 'flex', gap: 8 }}>
            <button className="mo-pill mo-tap" style={{
      ...
      <div style={{ padding: '18px 22px 0', display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
        <div className="mo-mono" style={{ fontSize: 10, letterSpacing: '0.25em', color: 'rgba(200,210,230,0.55)' }}>
          RECENT INSIGHTS
        </div>
      ...
      <div style={{ padding: '10px 18px 0', display: 'flex', flexDirection: 'column', gap: 8 }}>
        {[
          ...
          <div key={i} className="mo-glass mo-tap" style={{ padding: 14 }}>
            <div className="mo-mono" style={{ fontSize: 9.5, letterSpacing: '0.28em', color: 'rgba(167,139,250,0.8)' }}>{c.tag}</div>
            <div className="mo-serif" style={{
              marginTop: 6, fontSize: 16, lineHeight: 1.3,
              color: 'rgba(245,247,255,0.95)', letterSpacing: '-0.01em',
            }}>{c.t}</div>
            <div style={{
      ...
      <div style={{ padding: '14px 22px 0' }}>
        <div className="mo-glass" style={{ padding: 14, display: 'flex', alignItems: 'center', gap: 14 }}>
          <div style={{ flex: 1 }}>
            <div className="mo-mono" style={{ fontSize: 9.5, letterSpacing: '0.28em', color: 'rgba(167,139,250,0.8)' }}>
              GROWTH FOCUS · WEEK 03
            </div>
            <div className="mo-serif" style={{
              marginTop: 4, fontSize: 16, lineHeight: 1.2,
              color: 'rgba(245,247,255,0.95)', letterSpacing: '-0.01em',
            }}>Letting things be unfinished.</div>
          </div>
          {/* mini sparkline */}
          <svg width="74" height="36" viewBox="0 0 74 36">
            <path d="M1 28 C 10 24, 14 26, 20 18 S 32 6, 40 14 S 56 22, 72 8"
              fill="none" stroke="rgba(167,139,250,0.95)" strokeWidth="1.5" strokeLinecap="round" />
            <path d="M1 28 C 10 24, 14 26, 20 18 S 32 6, 40 14 S 56 22, 72 8 L 72 34 L 1 34 Z"
              fill="url(#sparkfill)" opacity="0.35" />
            <defs>
      ...
      <div style={{
        ...
        {[
          ...
          <div key={t.l} className="mo-tap" style={{
...
// ═════════════════════════════════════════════════════════════
// 08 · Voice Conversation
// ═════════════════════════════════════════════════════════════
function Screen08_VoiceChat() {
  return (
    <div className="mo-screen">
      <div className="mo-ambient deep" />
      <div className="mo-grain" />
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '6px 22px' }}>
        <div className="mo-glass mo-tap" style={{
          ...
          <svg width="14" height="14" viewBox="0 0 14 14"><path d="M11 1L1 7l10 6V1z" fill="rgba(245,247,255,0.95)"/></svg>
        </div>
        <div style={{ textAlign: 'center' }}>
          <div className="mo-mono" style={{ fontSize: 9.5, letterSpacing: '0.3em', color: 'rgba(167,139,250,0.85)' }}>LIVE · MIRROR</div>
          <div style={{
            marginTop: 4, fontFamily: 'Geist', fontSize: 13, fontWeight: 500,
      ...
      {/* AI reflection caption */}
      <div style={{
        position: 'absolute', top: 360, left: 28, right: 28, textAlign: 'center',
      }}>
        <div className="mo-mono" style={{ fontSize: 9.5, letterSpacing: '0.3em', color: 'rgba(167,139,250,0.85)' }}>MIRROR · LISTENING</div>
        <div className="mo-serif" style={{
          marginTop: 12, fontSize: 22, lineHeight: 1.3,
          color: 'rgba(245,247,255,0.95)', letterSpacing: '-0.015em',
          textWrap: 'pretty',
        }}>
      ...
      {/* waveform */}
      <div style={{
        position: 'absolute', bottom: 168, left: 32, right: 32,
        display: 'flex', justifyContent: 'center',
      }}>
        <div className="mo-wave" style={{ width: '100%', justifyContent: 'space-between' }}>
          {Array.from({ length: 40 }).map((_, i) => (
            <i key={i} style={{
              ...
              opacity: 0.35 + (Math.sin(i / 2) + 1) * 0.3,
            }} />
          ))}
        </div>
      </div>
      ...
      <div style={{
        ...
        <div className="mo-tap" style={{
          ...
          boxShadow:
            ...
            '0 12px 36px rgba(180,30,50,0.4)',
          display: 'flex', alignItems: 'center', justifyContent: 'center',
        }}>
        ...
        <div className="mo-glass mo-tap" style={{
          ...
          <svg width="16" height="18" viewBox="0 0 16 18" fill="none">
            <path d="M8 2v14M2 8l6-6 6 6" stroke="rgba(245,247,255,0.9)" strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round" transform="rotate(90 8 9)"/>
            <rect x="1" y="11" width="14" height="6" rx="3" stroke="rgba(245,247,255,0.9)" strokeWidth="1.2" fill="none"/>
          </svg>
        </div>
      </div>
    </div>
...
// 09 · Text Journaling
// ═════════════════════════════════════════════════════════════
function Screen09_Journal() {
  const [text, setText] = React.useState(
    'I keep coming back to that conversation. The one where I went quiet and he kept talking. I don\'t know if I was protecting him or hiding from myself.'
  );
  return (
    <div className="mo-screen">
      ...
      {/* AI nudge */}
      <div style={{ position: 'absolute', bottom: 168, left: 22, right: 22 }}>
        <div className="mo-glass" style={{ padding: 14, position: 'relative' }}>
          ...
          <div style={{
            marginTop: 6, fontFamily: 'Instrument Serif', fontSize: 16, lineHeight: 1.35,
            color: 'rgba(245,247,255,0.92)', letterSpacing: '-0.01em',
          }}>
            "Protecting" and "hiding" — would you tell them apart in your body, or do they feel the same?
          </div>
          <div style={{ display: 'flex', gap: 6, marginTop: 12 }}>
            <span className="mo-tap" style={{
              fontFamily: 'Geist', fontSize: 11.5, padding: '6px 12px', borderRadius: 999,
              background: 'rgba(255,255,255,0.06)', color: 'rgba(245,247,255,0.85)',
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', gap: 4 }}>
          {[
            <path key="b" d="M3 2h5a3 3 0 010 6H3V2zm0 6h6a3 3 0 010 6H3V8z" stroke="rgba(245,247,255,0.7)" strokeWidth="1.4" fill="none" strokeLinejoin="round"/>,
            <path key="i" d="M6 2h6M6 14h6M9 2v12" stroke="rgba(245,247,255,0.7)" strokeWidth="1.4" strokeLinecap="round"/>,
            <path key="q" d="M3 4l3 8M9 4l-3 8M11 8h4" stroke="rgba(245,247,255,0.7)" strokeWidth="1.4" strokeLinecap="round"/>,
          ].map((p, i) => (
  ...
  return (
    <div className="mo-screen">
      ...
      <div style={{ padding: '20px 28px 0' }}>
        ...
        <h1 className="mo-serif" style={{
          margin: '12px 0 0', fontSize: 30, lineHeight: 1.12,
          color: 'rgba(245,247,255,0.98)', letterSpacing: '-0.025em',
        }}>
      ...
      <div style={{ padding: '20px 18px 0', display: 'flex', flexDirection: 'column', gap: 10 }}>
        {[
          {
            tag: 'EMOTION',
            t: 'Beneath the calm: a low, steady grief.',
            d: 'You named protection. The body sounded more like resignation.',
          },
          {
            tag: 'PATTERN',
            t: 'Third time this month: silence as kindness.',
            d: 'It works in the moment. It charges interest by morning.',
          },
          {
            tag: 'REFRAME',
            t: 'What if staying soft included staying honest?',
            d: 'Saying it might be the gentlest thing — for both of you.',
          },
        ].map((c, i) => (
          <div key={i} className="mo-glass" style={{ padding: 16, position: 'relative', overflow: 'hidden' }}>
            ...
            <div className="mo-serif" style={{
              marginTop: 10, fontSize: 18, lineHeight: 1.25,
              color: 'rgba(245,247,255,0.95)', letterSpacing: '-0.015em',
            }}>{c.t}</div>
            <div style={{
              marginTop: 6, fontFamily: 'Geist', fontSize: 12.5, lineHeight: 1.5,
              color: 'rgba(220,225,240,0.6)', fontWeight: 300,
            }}>{c.d}</div>
          </div>
        ))}
      </div>
      ...
      <div style={{
        ...
        <button className="mo-pill mo-tap" style={{ flex: 1, height: 50, fontSize: 14 }}>Save</button>
        <button className="mo-pill primary mo-tap" style={{ flex: 1, height: 50, fontSize: 14 }}>
          Talk it through
        </button>
      </div>
    </div>
...
Object.assign(window, {
  Screen01_Splash, Screen02_Intro, Screen03_Account, Screen04_Intake, Screen05_Goals,
  Screen06_Voice, Screen07_Home, Screen08_VoiceChat, Screen09_Journal, Screen10_Reflection,
});
